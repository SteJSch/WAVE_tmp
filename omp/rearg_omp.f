*CMZ :  4.01/03 01/06/2023  08.08.23  by  Michael Scheer
*CMZ :  4.00/13 07/11/2021  17.22.27  by  Michael Scheer
*CMZ :  4.00/04 29/07/2019  16.46.25  by  Michael Scheer
*CMZ :  3.05/03 16/05/2018  16.05.00  by  Michael Scheer
*CMZ :  3.05/01 07/05/2018  16.28.53  by  Michael Scheer
...
*-- Author : Michael Scheer
      SUBROUTINE REARG_omp(ISOUR,IOBSV,nreromp,reromp)
*KEEP,GPLHINT.
*KEND.
...
C HENCE TIME OF X(I) SHOULD BE:
        ...
        IF (T/DT.GT.1.D7) THEN
          WRITE(LUNGFO,*)
          WRITE(LUNGFO,*)'*** ERROR IN REARG_OMP ***'
          WRITE(LUNGFO,*)
     ...
     &        ) THEN
          ...
          IF (IRANGE.EQ.1.AND.IAPERT.EQ.1) THEN
            ...
            IF (IVELOFIELD.EQ.0) THEN
              ...
              reromp(1,IPOIR)=(RNY*PZ-RNZ*PY)*DOM1+(RNX-BX)*DOM2
              reromp(2,IPOIR)=(RNZ*PX-RNX*PZ)*DOM1+(RNY-BY)*DOM2
              reromp(3,IPOIR)=(RNX*PY-RNY*PX)*DOM1+(RNZ-BZ)*DOM2
            ELSE IF (IVELOFIELD.EQ.1) THEN
              reromp(1,IPOIR)=(RNY*PZ-RNZ*PY)*DOM1
              reromp(2,IPOIR)=(RNZ*PX-RNX*PZ)*DOM1
              reromp(3,IPOIR)=(RNX*PY-RNY*PX)*DOM1
            ELSE IF (IVELOFIELD.EQ.2) THEN !approximation, see souintana
              reromp(1,IPOIR)=RNBX*R1
              reromp(2,IPOIR)=RNBY*R1
              reromp(3,IPOIR)=RNBZ*R1
            ELSE IF (IVELOFIELD.LT.0) THEN
              DOM2=CLIGHT1*DOM1*R1/(DMYGAMMA*DMYGAMMA)
              reromp(1,IPOIR)=(RNX-BX)*DOM2
              reromp(2,IPOIR)=(RNY-BY)*DOM2
              reromp(3,IPOIR)=(RNZ-BZ)*DOM2
            ELSE  !IVELOFIELD
              PRINT*, '*** ERROR IN REARG_OMP: BAD VALUE OF IVELOFIELD'
              STOP
            ENDIF !IVELOFIELD

C150793             reromp(4,IPOIR)=T+R*C1
            reromp(4,IPOIR)=T+(R-R0)*C1

            IF (IVELOFIELD.NE.2) THEN

              reromp(5,IPOIR)=(
     &          reromp(1,IPOIR)*reromp(1,IPOIR)
     &          +reromp(2,IPOIR)*reromp(2,IPOIR)
     &          +reromp(3,IPOIR)*reromp(3,IPOIR)
     &          )/BET1N
     ...
     &          )/BET1N
            ...
            reromp(6,IPOIR)=DT
            reromp(7,IPOIR)=BET1N
            reromp(8,IPOIR)=-CLIGHT1*R1
     ...
     &        -(RNX*BPX+RNY*BPY+RNZ*BPZ)/2.0D0
          ...
          ELSE
            ...
            IF (IAPERT.EQ.0.AND.IWARN.EQ.0) THEN
              ...
              WRITE(LUNGFO,*)'*** WARNING REARG_OMP:'
              WRITE(LUNGFO,*)
     &          'APERTURE AFFECTS SPECTRUM CALCULATION FOR SOURCE'
     &          ,ISOUR
              WRITE(LUNGFO,*)
              WRITE(6,*)'*** WARNING REARG_OMP:'
              WRITE(6,*)
     &          'APERTURE AFFECTS SPECTRUM CALCULATION FOR SOURCE'
     &          ,ISOUR
              WRITE(6,*)
            ...
            reromp(4,IPOIR)=T+(R-R0)*C1
            reromp(5,IPOIR)=0.0d0
            reromp(6,IPOIR)=DT

            reromp(7,IPOIR)=0.
            reromp(8,IPOIR)=0.

            reromp(9:11,IPOIR)=0.0d0
